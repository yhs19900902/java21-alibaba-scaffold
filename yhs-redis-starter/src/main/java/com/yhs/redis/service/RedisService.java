...
/**
 * @author 07664-linwei
 * @version Id: RedisRepository.java, v 0.1 2022/4/20 9:40 lw Exp $
 */
@Slf4j
@AllArgsConstructor
public class RedisService {
    ...
    public <T> T get(final String key) {
        byte[] rawKey = rawKey(key);
        return (T) redisTemplate.execute(connection -> deserializeValue(connection.get(rawKey), redisTemplate.getValueSerializer()), true);
    }
     ...
     * @return true成功 false 失败
     */
    public boolean setNxAndExpire(String key, Object value, Long time) {
    ...
    public Long incr(String key, Long delta) {
        if (delta < 0) {
            //  throw new ZipException(ExceptionCode.REDIS_INCR.getMsg());
        }
    ...
    public <T> T hget(String key, String item, RedisSerializer valueSerializer) {
        byte[] rawKey = rawKey(key);
        byte[] itemKey = rawKey(item);
        return (T) redisTemplate.execute(connection -> deserializeValue(connection.hGet(rawKey, itemKey), valueSerializer), true);
    }
     ...
     * @return 值
     */
    public <T> T hmget(String key, String... item) {
        return (T) redisTemplate.opsForHash().multiGet(key, Arrays.asList(item));
    }
    ...
    public boolean batchHmset(String key, Map<?, ?> map) {
        LinkedHashMap<byte[], byte[]> hashes = new LinkedHashMap<>(map.size());
        RedisSerializer hashValueSerializer = redisTemplate.getHashValueSerializer();
        map.forEach((k, v) -> hashes.put(rawKey(k), hashValueSerializer.serialize(v)));
        try {
            this.redisTemplate.executePipelined(new RedisCallback() {
                @Override
                public Object doInRedis(RedisConnection connection) throws DataAccessException {
                    connection.hMSet(RedisService.this.rawKey(key), hashes);
                    return null;
                }
            });
            return true;
    ...
    public <T> List<T> batchHmget(String key, List<String> items) {
        byte[][] bytes = items.
                stream().map(s -> rawKey(s))
        ...
        return (List<T>) redisTemplate.executePipelined(new RedisCallback<T>() {
            ...
            public T doInRedis(RedisConnection connection) throws DataAccessException {
                connection.hMGet(rawKey(key), bytes);
                return null;
            }
        });
    }
    ...
    public Boolean hset(String key, String item, Object value, RedisSerializer serializer) {
        try {
            redisTemplate.execute(connection -> connection.hSet(rawKey(key), rawKey(item), rawValue(value, serializer)), true);
            return true;
        } catch (Exception e) {
            log.error(": {}", e.getMessage());
            return false;
    ...
    public <T> List<T> getList(String key, int start, int end) {
        byte[] rawKey = rawKey(key);
        return (List<T>) redisTemplate.execute(connection -> deserializeValues(connection.lRange(rawKey, start, end), redisTemplate.getValueSerializer()), true);
    }
    ...
    private <T> List<T> deserializeValues(List<byte[]> rawValues, RedisSerializer<T> valueSerializer) {
    ...
    private <T> T deserializeValue(byte[] value, RedisSerializer<T> valueSerializer) {
        if (valueSerializer == null) {
            return null;
        }
