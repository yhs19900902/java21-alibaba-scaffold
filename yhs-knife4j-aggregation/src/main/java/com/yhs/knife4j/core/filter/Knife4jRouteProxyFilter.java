...
/**
 * @author 07664-linwei
 * @version Id: Knife4jRouteProxyFilter.java, v 0.1 2023/7/25 14:13 lw Exp $
 */
public class Knife4jRouteProxyFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (routeDispatcher.checkRoute(request.getHeader(RouteDispatcher.ROUTE_PROXY_HEADER_NAME))) {
            if (CharSequenceUtil.endWith(uri, RouteDispatcher.OPENAPI_GROUP_INSTANCE_ENDPOINT)) {
            ...
            //go on
            if (CharSequenceUtil.endWith(uri, RouteDispatcher.OPENAPI_GROUP_ENDPOINT)) {
                //响应当前服务聚合结构
                writeRouteResponse(response, gson.toJson(routeDispatcher.getRoutes()));
            } else if (CharSequenceUtil.endWith(uri, RouteDispatcher.OPENAPI_GROUP_INSTANCE_ENDPOINT)) {
                String group = request.getParameter("group");
                SwaggerRoute swaggerRoute = routeDispatcher.getRoute(group);
                writeRouteResponse(response, swaggerRoute == null ? "" : swaggerRoute.getContent());
                //响应当前服务disk-实例
